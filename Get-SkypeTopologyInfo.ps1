...
##################################################
#
#  Get-SkypeTopologyInfo.ps1
#
#  Author: Alejandro
#  Version: 1.3
#  Release: 09/2011
#  
#  Based on the Get-LyncTopologyInfo.ps1 script by the Author below, tweaked for newer OS and Excel/Visio versions:
#  Author: Christopher Cook
#  Website: EmptyMessage.com
#  Version: 1.2
#  Release: 09/2011
#  https://emptymessage.com/?p=49#more-49
...
#$ExcelFileName = $TBXMLFileName.substring(0,$TBXMLFileName.Length - 6) + " Test - Topology.XLSX"
...
#ac - sets Excel instance to current workbook and creates 1 empty sheet in the workbook
$Workbook = $ExcelApp.Workbooks.Add()
#ac - adds blank worksheet, switches to it
...
#$Worksheets = $WorkBook.Worksheets.Add()
#ac - to delete specify sheet first
#$Worksheets = $WorkBook.Worksheets.item("Sheet2").Delete()

#ac - use this add method to not create overlap
#$WorkBook.Worksheets.add([System.Reflection.Missing]::Value,$WorkBook.Worksheets.Item($WorkBook.Worksheets.count))
#[System.Reflection.Missing]::Value,$WorkBook.Worksheets.Item(1)

#Name the sheets in the Workbook and assign variables to them.
$SitesServersSheet = $Workbook.Worksheets.Item(1)
$SitesServersSheet.Name = "Skype Sites And Servers"
#ac - this line is causing error
#ac - have to add or else the sheet is missing and causes error
#$Worksheets = $WorkBook.Worksheets.Add()
...
#[System.RunTime.IntropServices.Marshall]::ReleaseComObject ($ExcelApp)
...
#ac - netw_u is for the network and peripherals shapes, path is 'C:\Program Files\Microsoft Office\root\Office16\Visio Content\1033'

$objDocument = $objDocuments.Add("NETW_U.VST")
#ac - test with "NETSYM_U.VSSX" - creates a stencil, not template
#$objDocument = $objDocuments.Add("NETSYM_U.VSSX")
...
#ac - error here - fixed by manually downloading file and adding 'My Shapes' folder (https://www.microsoft.com/en-us/download/confirmation.aspx?id=20891)
...
#ac - no errors here, sets all the Server types to the appropiate shape
...
#ac - commented by default but no reason is given as to why, testing
...
#ac - above all works, creates two rectangles for 'Internet' and 'DMZ'. Leaving commented because there is nothing to fill the objects and blocks the sites and servers list

#ac - condensing text
$LegendText = @()
$ClusterServers
$LegendText += $DefaultSIPDomain + " | Skype Topology`n"
$LegendText += $Sites.Count.ToString() + " Sites | "
$LegendText += $Servers.Count.ToString() + " Servers | "
#$LegendText += $SIPDomains.Count.ToString() + " SIP Domains | "
#if ($Clusters){$LegendText += $Clusters.Count.ToString() + " Clusters`n"}
$LegendText += "`n"
$LegendText += $SIPDomains.Count.ToString() + " SIP Domain(s):`n"
$SIPDomains | foreach-object{$LegendText += $_ + "`n"}
$LegendText += "`n"
if ($Clusters){
$LegendText +=  $Clusters.Count.ToString()+" Cluster(s):`n"
...
#ac - error here, changed to 'ToString()' and didn't get error but just printed '[system.object]' on screen
#$shpLegendBG.Text =  $LegendText
#ac - test with foreach, works but need to format
foreach ($object in $LegendText){$shpLegendBG.Text += $object}
#$shpLegendBG.TextStyle | Get-Member *

#ac - testing to breakpoint here
...
#ac - removing comment
 ...
 #ac - loop starts here to draw each server
...
foreach ($Service in $Services){if ($Service.InstalledOn -eq $Server.UniqueID){$ServerServices += $Service.RoleName}}
	...
	#ac - error here but only after passing through multiple times
	#draws all front end servers succesffully, errors are when there is no object
	if ($Null -ne $shpServerType ) {$shpServer = $objPage.Drop($shpServerType, $ShapeHorizontalCenter, $ShapeVerticalCenter) }
	#ac - error here, changing from $Server to $Server.Name
	$shpServer.Name = $Server.Name
